...
`mark-tmp` adds the `tmp` tag to one/more cache entries without deleting the cached files.

**Syntax**

```bash
mlc mark-tmp cache --tags=<list_of_tags_used_while_running_script>
```

This is useful when a cache needs to be regenerated by future runs while keeping the existing files around for inspection or partial reuse.
